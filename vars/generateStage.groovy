    ...
    stage("Test - ${test_config.label} - ${test_config.cuda_ver} - ${test_config.py_ver} - ${test_config.os}") {
      node(test_config.label) {
        docker
          ...
          .inside("--privileged --runtime=nvidia -e NVIDIA_VISIBLE_DEVICES=$EXECUTOR_NUMBER -e ARC=${test_config.arc} -e CUDA=${test_config.cuda_ver} -e PY_VER=${test_config.py_ver}") {
          steps()
        }
      }
    }
